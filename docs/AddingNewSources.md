...
Biolink output type. It also carries out other steps, such as filtering out prefixes that
are not designated valid for that Biolink type, choosing a preferred label, and generating
a synonyms entry to be loaded into NameRes.
...
To resolve a CURIE to its URL, use `src/util.py:get_biolink_prefix_map()`, which returns a
`curies.Converter` built from the prefix map for the configured Biolink version (it already
handles the duplicate-entry quirk that otherwise requires `strict=False`):

```python
from src.util import get_biolink_prefix_map
...
- Network-marked tests in `tests/datahandlers/` that exercise downloads.
- A pipeline-marked test in `tests/pipeline/test_<source>_pipeline.py` that uses the
  ...
  - The goal behind pipeline tests are to exercise multiple parts of the pipeline using
    Snakemake: existing downloads are reused unless explicitly cleaned, so that only a
    particular failing part of a pipeline can be isolated and worked on at a time.
...
and a "write compendia" wrapper, then register the helper in `PIPELINE_CONFIG` in
...
4. **Clique impact** — for each semantic type with a registered compute hook, counts of:
   ...
   - **existing cliques that contain source identifiers in the after state**, split into
     ...
     - cliques where the source CURIE was already pulled in via another source's xref;
       ...
       case interchangeably — the dataclass field is `preexisting_source_curies`),
...
type is not yet registered in `PIPELINE_CONFIG` or the intermediate files were not
fully present.
...
- **`new-cliques.csv`** — one row per pure-new clique the source introduces. Columns:
  `pipeline, preferred_id, preferred_label, biolink_type, member_count,
...
filters every member's prefix against — not each identifier's own declared type (the
detail files keep the identifier's declared type in a separate column for context):
`would_be_added` is `true`/`false`/blank (blank means unknown — no clique type, or the
report ran with `--no-biolink-lookup`), and `needs_biolink_registration = true` flags an
identifier whose prefix must be added to the Biolink Model for that class before Babel can
ever emit it. EMAPA's gross-anatomy terms are
...
- **`new-xrefs.tsv`** — one row per cross-reference touching a source CURIE, scanned across
  *all* concord files (not just the source's own — a source's xrefs frequently live in
  ...
  `pipeline, subject, subject_label, predicate, object, object_label, asserted_by,
  ...
  own concord asserts the xref (a brand-new bridge) or `from_other_source` when another
  source's concord asserts a row that happens to touch a source CURIE. This writer does
  not diff the pre-source glom state, so `from_other_source` does not imply the xref is
  newly activated by this source — it may have already existed before the source was added.
...
- `src/cli/source_impact_report.py` — argparse entrypoint, semantic-type dispatch
  (`PIPELINE_CONFIG`), the synthetic re-glom loop, the remote-comparison loop, and
...
`PIPELINE_CONFIG` has one entry, for anatomy. Adding a source that touches
...
- `src/cli/source_impact_report.py` — the CLI implementation; `PIPELINE_CONFIG` is
