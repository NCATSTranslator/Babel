...
- `otherdesignations` behaves **differently**: there `''` and commas *do* co-occur (e.g. 459 rows
  ...
  comma-split. That field is never shredded — every `''` in it is genuine double-prime nomenclature
  (next section).
...
### The middle pieces, and how they were removed

Dropping the two `''`-marked ends still left the value's *middle* pieces behind — `family 706`,
`subfamily A`, `MET`, even a bare `3`. These carry no `''` at all, so nothing about their shape
marks them as junk; they look exactly like ordinary aliases. Over the full file that was **912 junk
synonyms across the 276 rows, 186 distinct tokens** — and several are worse than merely useless,
since `MET`, `CYS` and `PRO` are real human gene symbols.

The same fact that makes the value unrecoverable also makes them removable: `Full_name` holds the
value intact and correctly comma-formatted, so its comma-pieces are *exactly* the fragments to
delete. `split_ncbigene_synonym_field` now takes that value and drops them — but only when the field
carries an open marker, since absent one nothing was shredded into it and every fragment is real.
Fixed in [issue #932](https://github.com/NCATSTranslator/Babel/issues/932); every removed synonym is
listed in [`shredded_pieces.csv`](./shredded_pieces.csv), generated by
[`shredded_pieces_report.py`](./shredded_pieces_report.py) — see
[`shredded_pieces_report.md`](./shredded_pieces_report.md) for the summary.

## Deferred follow-up questions

Still open (each a likely next script here):
