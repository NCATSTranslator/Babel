...
* The `metadata/` directory contains metadata reports for every compendium file. It is generated from the metadata files
  for intermediate outputs for each file and is intended to capture all the provenance information for every Babel run,
...
* The `logs/` files are produced as the pipeline runs, although logs for runs that succeed are deleted to save space.
* The `intermediate/` files are produced as the pipeline runs, and may provide clues as to how the outputs were generated.
