...
rule generate_mapping_sources_table:
    input:
        metadata_yaml_files = expand("{od}/metadata/{compendia_filename}.yaml", od=config["output_directory"], compendia_filename=map(lambda s: str(s), get_all_compendia(config)))
    output:
