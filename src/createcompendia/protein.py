...
def write_umls_ids(mrsty, outfile):
    # The UMLS semantic-type -> Biolink-class assignments for proteins live in the central registry
    # src/datahandlers/umls/semantic_types.py (see UMLS_TYPE_MAP, compendium="protein"). That
    # registry also enforces that no UMLS tree number lands in both protein and chemicals: the
    # protein-owned trees (Amino Acid/Peptide/Protein T116, Receptor T192, Enzyme T126) are excluded
    # from chemicals.py via its local blocklist.
    umls.write_umls_ids(mrsty, ust.category_map_for("protein"), outfile)
