...
- **NCBIGene synonyms lost to a quoting bug are back**
  ([Babel #917](https://github.com/NCATSTranslator/Babel/pull/917)). The previous parse of
  `gene_info.gz`'s synonym field discarded roughly 4,000 real synonyms -- among them genuine
  double-prime nomenclature such as RNA polymerase `beta''` -- so NameRes lookups by those names
  failed in earlier builds. If a gene search came back empty against 2025sep1 or earlier, retry it.
