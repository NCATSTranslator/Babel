...
<!-- Wrong behaviour a consumer may have relied on, now corrected: "you used to see X, you now see
     ...
     want." Candidates are listed in `## Suggested deletions` at the foot of this note; promote the
     ones worth flagging and delete this section if none are. -->
...
- Babel now produces a GeneProteinConflated synonyms file so that NameRes Solr applies the same
...
- `TaxonFactory` now holds its taxon table in SQLite rather than entirely in memory, via a new
  generic `TSVSQLiteLoader` [Babel #482](https://github.com/NCATSTranslator/Babel/pull/482).
  `SynonymFactory` and `DescriptionFactory` are still in-memory.
...
- The `biolink:Food` retype in [Babel #918](https://github.com/NCATSTranslator/Babel/pull/918)
  ...
  ([Babel issue #935](https://github.com/NCATSTranslator/Babel/issues/935),
  [Babel issue #948](https://github.com/NCATSTranslator/Babel/issues/948)). The fix
...
Explanations for the movements in the table above. This is a large release -- 605.9M CURIEs against
...
- [ ] **Polypeptide is down 97.0%** (166 -> 5) -- unexplained. Tiny in absolute terms, but no PR in
  this release obviously accounts for it. Investigate or file an issue before publishing.

## Suggested deletions

<!-- Assembled by the final review pass (see .claude/skills/release-notes/SKILL.md, step 7). These
     are candidates only: each says where it came from and why. Promote anything worth keeping back
     into the sections above, then delete this section before publishing. -->

Cited in an earlier release note, so these describe changes that already shipped:

- **GeneProteinConflated synonyms file**
  ([Babel #486](https://github.com/NCATSTranslator/Babel/pull/486)) — already in
  [2025sep1](./2025sep1.md) as a `[MAJOR]` entry.
- **ChEBI secondary identifiers** ([Babel #473](https://github.com/NCATSTranslator/Babel/pull/473))
  — already in [2025sep1](./2025sep1.md), described there as an identifier-based property store.
- **`TaxonFactory` on SQLite** ([Babel #482](https://github.com/NCATSTranslator/Babel/pull/482)) —
  already in [2025sep1](./2025sep1.md).
- **Metadata YAML files** ([Babel #459](https://github.com/NCATSTranslator/Babel/pull/459)) — in
  [v1.11](./v1.11.md), which was never deployed, so this may never have reached a shipped note.
- **config.json replaced by config.yaml**
  ([Babel #460](https://github.com/NCATSTranslator/Babel/pull/460)) — likewise in
  [v1.11](./v1.11.md) only. The bullet in this note did not cite a PR; #460 is it.
- **UniChem xref filtering** ([Babel #508](https://github.com/NCATSTranslator/Babel/pull/508)) —
  likewise in [v1.11](./v1.11.md) only, described there in more detail.

Repository housekeeping with no effect on Babel's output or how it is used:

- **GitHub issue templates**
  ([Babel #663](https://github.com/NCATSTranslator/Babel/pull/663),
  [Babel #710](https://github.com/NCATSTranslator/Babel/pull/710),
  [Babel #712](https://github.com/NCATSTranslator/Babel/pull/712),
  [Babel #716](https://github.com/NCATSTranslator/Babel/pull/716),
  [Babel #724](https://github.com/NCATSTranslator/Babel/pull/724)).
...
Candidates for `## Bugfixes`, if you think a consumer should check their analyses against them:

- **NCBIGene synonym-field quoting**
  ([Babel #917](https://github.com/NCATSTranslator/Babel/pull/917)) — the previous parse discarded
  roughly 4,000 real synonyms, so NameRes lookups by those names failed in earlier builds.
- **ChEBI SDF tag renames** ([Babel #951](https://github.com/NCATSTranslator/Babel/pull/951)) —
  silently emptied the secondary-ID and PubChem ingests, so ChEBI cliques in earlier builds were
  missing xrefs they should have had.
- **The `biolink:Food` clique-level retype** — already written up under Known issues, and the
  clearest "you saw X, you now see Y" case in this release.
