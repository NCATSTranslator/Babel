...
- [MAJOR] Improved DrugChemical conflation order
  ...
  - Previously, we would attempt to pick the best DrugChemical clique leader we could, and then
    ...
    clique leader we picked had that type, even though we prefer SmallMolecule.
  - This PR significantly changes this algorithm so that we now always go in the preferred
    conflation type order (i.e. SmallMolecule, MolecularMixture, etc.), choose all the identifiers
    of that type, and arrange them in the best possible order, which at the moment is:
