...
# ---------------------------------------------------------------------------
# Disease pipeline fixtures
# ---------------------------------------------------------------------------
#
# The disease compendium is assembled from many vocabularies (MONDO, DOID, Orphanet,
# HP, MESH, NCIT, UMLS, OMIM, EFO), so the disease fixtures are keyed by compendium
# (one) rather than by vocabulary (many) -- this is the opposite shape from
# VOCABULARY_REGISTRY and is deliberately kept orthogonal.
#
# All disease ID rules are delegated to Snakemake via pipeline_output so that each
# rule's downloads and prerequisites are handled the way they are in production --
# avoiding the need to duplicate write_*_ids parameter wiring in this file for a
# nine-vocabulary compendium.  A prior full pipeline run on disk makes the fixtures
# instant (each pipeline_output call early-exits when its output file is present).


DISEASE_VOCAB_TO_RULE = {
    "MONDO":    "disease_mondo_ids",
    "DOID":     "disease_doid_ids",
    "Orphanet": "disease_orphanet_ids",
    "HP":       "disease_hp_ids",
    "MESH":     "disease_mesh_ids",
    "NCIT":     "disease_ncit_ids",
    "UMLS":     "disease_umls_ids",
    "OMIM":     "disease_omim_ids",
    "EFO":      "disease_efo_ids",
}


@pytest.fixture(scope="session")
def disease_id_pipeline_outputs(pipeline_output):
    """Ensure every babel_outputs/intermediate/disease/ids/<VOCAB> file is present.

    Returns ``{vocab: path}`` over all nine disease vocabularies.  Each entry is
    backed by ``pipeline_output("disease_<vocab>_ids", path)`` so Snakemake handles
    downloads (DOID labels, Orphanet labels, EFO efo.owl) and UberGraph queries.
    Any vocab whose rule fails causes pytest.skip via pipeline_output.
    """
    from src.util import get_config
    cfg = get_config()
    base = os.path.join(cfg["intermediate_directory"], "disease", "ids")
    return {
        vocab: pipeline_output(rule, os.path.join(base, vocab))
        for vocab, rule in DISEASE_VOCAB_TO_RULE.items()
    }


@pytest.fixture(scope="session")
def disease_concords_dir(pipeline_output):
    """Ensure disease concord files are present; return the concords directory path.

    Uses ``disease_manual_concord`` as the sentinel -- it copies a local file
    (``input_data/manual_concords/disease.txt``) with no downloads or compute,
    analogous to ``get_chemical_wikipedia_relationships`` for chemicals.

    Heavier concord files (MONDO/HP from get_disease_obo_relationships, EFO, UMLS,
    DOID) are used if already present from a prior full pipeline run but are not
    auto-generated by this fixture.
    """
    from src.util import get_config
    cfg = get_config()
    concords_dir = os.path.join(cfg["intermediate_directory"], "disease", "concords")
    sentinel = os.path.join(concords_dir, "Manual")
    pipeline_output("disease_manual_concord", sentinel)
    return concords_dir


@pytest.fixture(scope="session")
def disease_mondoclose(pipeline_output):
    """Ensure intermediate/disease/concords/MONDO_close is present; return its path.

    MONDO_close is one of the outputs of ``get_disease_obo_relationships`` and is
    needed by ``compute_disease_cliques`` (it feeds the ``close=`` argument to glom).
    """
    from src.util import get_config
    cfg = get_config()
    mondoclose = os.path.join(cfg["intermediate_directory"], "disease", "concords", "MONDO_close")
    return pipeline_output("get_disease_obo_relationships", mondoclose)


@pytest.fixture(scope="session")
def disease_cliques(disease_id_pipeline_outputs, disease_concords_dir, disease_mondoclose):
    """Run compute_disease_cliques() over the on-disk disease concord and ID files.

    Returns ``(curie_to_clique, curie_to_type)``:
      - ``curie_to_clique[curie]`` is the frozenset of CURIEs in that clique.
      - ``curie_to_type[curie]`` is the Biolink type assigned by create_typed_sets
        (``biolink:Disease`` or ``biolink:PhenotypicFeature``).

    Session-scoped so glom runs once per pytest invocation.
    """
    ...
    badxrefs = {
        "HP":    os.path.join(input_dir, "badHPx.txt"),
        "MONDO": os.path.join(input_dir, "mondo_badxrefs.txt"),
        "UMLS":  os.path.join(input_dir, "umls_badxrefs.txt"),
    }

    identifiers = list(disease_id_pipeline_outputs.values())
    concordances = []
    for entry in os.scandir(disease_concords_dir):
        # Skip subdirectories, YAML metadata, and the MONDO_close file (passed
        # separately to compute_disease_cliques).
        if not entry.is_file() or entry.name.endswith(".yaml") or entry.name == "MONDO_close":
            continue
        concordances.append(entry.path)

    typed_sets = compute_disease_cliques(concordances, identifiers, disease_mondoclose, badxrefs)
    ...
    for biotype, sets in typed_sets.items():
        for clique in sets:
            for curie in clique:
                curie_to_clique[curie] = clique
                curie_to_type[curie] = biotype
    return curie_to_clique, curie_to_type


@pytest.fixture(scope="session")
def babel_node_factory():
    """A single NodeFactory wired to babel_downloads/ for label resolution.

    Network-dependent on first use: bmt.Toolkit fetches biolink-model.yaml from
    GitHub.  Skips if instantiation fails.  Session-scoped so the model fetch and
    common-labels loading are paid once.
    """
    try:
        from src.node import NodeFactory
        from src.util import get_config
        cfg = get_config()
        return NodeFactory(make_local_name(""), cfg["biolink_version"])
    except Exception as e:
        pytest.skip(f"Could not instantiate NodeFactory (Biolink Model fetch?): {e}")


def simulate_label(curie, clique, node_type, node_factory):
    """Return the preferred label _select_preferred_label() would choose for `clique`.

    Mirrors what write_compendium() does for one clique:
      1. node_factory.create_node() sorts identifiers by Biolink prefix order and
         applies labels via NodeFactory.apply_labels (which reads
         babel_downloads/<PREFIX>/labels and the configured common-labels files).
      2. _select_preferred_label() applies the same boost-prefix ordering and
         per-type length-demotion config that write_compendium uses.

    The `curie` argument is the CURIE the caller is asking about; it must be a
    member of `clique`.  It is accepted for symmetry with the test API (so the
    failure message can name it) but is not otherwise used here -- the algorithm
    picks one preferred label for the whole clique.
    """
    from src.babel_utils import _select_preferred_label
    from src.util import get_config
    cfg = get_config()
    node = node_factory.create_node(input_identifiers=list(clique), node_type=node_type, labels={})
    if node is None:
        return None
    types = node_factory.get_ancestors(node_type)
    return _select_preferred_label(
        node,
        types,
        cfg["preferred_name_boost_prefixes"],
        cfg.get("demote_labels_longer_than", {}),
    )
