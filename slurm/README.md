...
`uv sync` now needs a Rust toolchain on the login node: the build backend is maturin, which compiles
a native extension (see [`../rust/README.md`](../rust/README.md)). Without `cargo` on `$PATH`, uv
...
directory is its own problem — so install rustup (or `module load` a Rust toolchain) first. Only the
login node needs it: `uv sync` runs once there and the compute nodes reuse the resulting `.venv`
from `/projects`.
